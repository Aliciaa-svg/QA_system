...
import argparse
import time
from search_QA.util import get_corpus, word_tokenize, build_word_embedding
from search_QA.model_zoo.sif_model import SIFRetrievalModel

# 配置参数
parser = argparse.ArgumentParser(description='Information retrieval model hyper-parameter setting.')
parser.add_argument('--input_file_path', type=str, default='./search_QA/dream.xls', help='Training data location.')
...
# gensim模型路径
parser.add_argument('--gensim_model_path', type=str, default='./search_QA/cached/gensim_model.pkl')
parser.add_argument('--pretrained_gensim_embddings_file', type=str, default='./search_QA/cached/gensim_word_embddings.txt')
parser.add_argument('--cached_gensim_embedding_file', type=str, default='./search_QA/cached/embeddings_gensim.pkl')
...
def main(question):
    ...
    # sif模型
    sif_model = SIFRetrievalModel(questions, './search_QA/cached/gensim_word_embddings.txt',args.cached_gensim_embedding_file, args.embedding_dim)
    # 预测
    # res = predict(sif_model, query)
    # # 打印
    # print('Query: ', query)
    ...
    while flag:
        ...
        return questions_src[question_k[0]], answers[question_k[0]]
