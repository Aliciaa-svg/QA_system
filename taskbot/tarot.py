...
def tarot(input, dialog_status):
    ...
    if dialog_status.intent == 'tarot start':
        ...
        for i in range(4):
            idx = indices[i]
            meaning_key = meaning_keys[i]
            meaning = meanings[meaning_key]
            output.append(meaning_key)
            output.append(meaning)

            card_key = card_keys[idx-1]
            if isinstance(cards[card_key], dict):
                rand = random.randint(1, 2)
                if rand == 1: # 正位
                    output.append((str(card_key)+'正位'))
                    val = cards[card_key]["正位"]
                    
                else: # 逆位
                    val = cards[card_key]["逆位"]
                    output.append((str(card_key)+'逆位'))
            else:
                val = cards[card_key]
                output.append((str(card_key)))
            output.append(val)
