import re
import taskbot.ganzhi as ganzhi
import math
import numpy as np
...
def birth(input, cache, dialog_status):
    ...
    if dialog_status.bazi == "start":
        dialog_status.bazi = "year"
        return ("请说出你出生的年份")
    if dialog_status.bazi == "year":
        num = []
        for i in range(4):
        ...
        year = ind.join(num)
        cache.append(year)
        dialog_status.bazi = "month"
        return ("请说出你出生的月份")
    elif dialog_status.bazi == "month":
        month = char2num[input]
        cache.append(month)

        dialog_status.bazi = "day"
        return ("请说出你出生的日期")
    elif dialog_status.bazi == "day":
        day = char2num[input]
        cache.append(day)
        dialog_status.bazi = "time"
        return ("说出出生的时间")
    elif dialog_status.bazi == "time":
        time = char2num[input]
        cache.append(time)
        dialog_status.bazi = 'calculate bazi'
